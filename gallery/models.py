...
class GalleryCategories(models.Model):
    ...
    def update_category(id, search_term , new_cat):
        try:
            to_update = GalleryCategories.objects.get(name = search_term)
            to_update.name = new_cat
            to_update.save()
            return to_update
        except GalleryCategories.DoesNotExist:
...
class GalleryImages(models.Model):
    ...
    category = models.ManyToManyField(GalleryCategories)
    location = models.ForeignKey('GalleryLocations', on_delete=models.CASCADE, default=1)
...
class GalleryLocations(models.Model):
    ...
    def update_location(id, location , new_loc):
        try:
            to_update = GalleryLocations.objects.get(city = location)
            to_update.city = new_loc
            to_update.save()
            return to_update
        except GalleryLocations.DoesNotExist:
            print('Location you specified does not exist')

    def get_all(id):
        cities = GalleryLocations.objects.all()
        return cities